...
OUTPUT="${OUTPUT:-$PROJECT_ROOT/outputs/fast_wam_megatron_dcp_bf16_libero_2k_20260723}"
MANIFEST="${MANIFEST:-$PROJECT_ROOT/fast_wam/eval/manifest_libero_full_2k.json}"
...
DCP_METADATA="$DCP/.metadata"
if [[ -f "$DCP/latest_checkpointed_iteration.txt" ]]; then
  DCP_ITERATION="$(< "$DCP/latest_checkpointed_iteration.txt")"
  if [[ "$DCP_ITERATION" =~ ^[0-9]+$ ]]; then
    printf -v DCP_ITERATION_DIR "iter_%07d" "$DCP_ITERATION"
    DCP_METADATA="$DCP/$DCP_ITERATION_DIR/.metadata"
  fi
fi

if [[ ! -f "$DCP_METADATA" ]]; then
...
CUDA_VISIBLE_DEVICES="$EVAL_DEVICES" torchrun \
  ...
  --dtype bfloat16 \
  --n-action-steps 10 \
  --result-wait-timeout "$RESULT_WAIT_TIMEOUT" \
  --target-success-rate 0.976 \
  --resume
