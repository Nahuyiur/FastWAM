from __future__ import annotations

from dataclasses import dataclass


TASKVARS_TRAIN = [
    "push_button+0",
    "push_button+3",
    "push_button+4",
    "close_fridge+0",
    "close_laptop_lid+0",
    "close_microwave+0",
    "open_door+0",
    "open_box+0",
    "open_drawer+0",
    "open_drawer+2",
    "pick_and_lift+0",
    "pick_and_lift+2",
    "pick_and_lift+7",
    "pick_up_cup+8",
    "pick_up_cup+9",
    "pick_up_cup+11",
    "stack_blocks+30",
    "stack_blocks+36",
    "stack_blocks+39",
    "put_groceries_in_cupboard+0",
    "put_groceries_in_cupboard+3",
    "put_money_in_safe+0",
    "put_money_in_safe+1",
    "slide_block_to_color_target_peract+0",
    "slide_block_to_color_target_peract+1",
    "reach_and_drag_peract+14",
    "reach_and_drag_peract+18",
    "close_jar_peract+15",
    "close_jar_peract+16",
    "light_bulb_in_peract+17",
    "light_bulb_in_peract+19",
]

TASKVARS_TEST_L2 = [
    "push_button+13",
    "push_button+15",
    "push_button+17",
    "pick_and_lift+14",
    "pick_and_lift+16",
    "pick_and_lift+18",
    "pick_up_cup+10",
    "pick_up_cup+12",
    "pick_up_cup+13",
    "stack_blocks+24",
    "stack_blocks+27",
    "stack_blocks+33",
    "slide_block_to_color_target_peract+2",
    "slide_block_to_color_target_peract+3",
    "reach_and_drag_peract+5",
    "reach_and_drag_peract+7",
    "put_cube_in_safe+0",
    "close_jar_peract+3",
    "close_jar_peract+4",
    "light_bulb_in_peract+1",
    "light_bulb_in_peract+2",
    "lamp_on+0",
    "pick_and_lift_cylinder+0",
    "pick_and_lift_star+0",
    "pick_and_lift_moon+0",
    "pick_and_lift_toy+0",
    "put_groceries_in_cupboard+7",
    "put_groceries_in_cupboard+8",
]

TASKVARS_TEST_L3 = [
    "open_drawer_long+0",
    "open_drawer_long+1",
    "open_drawer_long+2",
    "open_drawer_long+3",
    "close_fridge2+0",
    "close_laptop_lid2+0",
    "close_microwave2+0",
    "open_door2+0",
    "open_box2+0",
    "open_drawer2+0",
    "open_drawer3+0",
    "close_door+0",
    "close_box+0",
    "toilet_seat_up+0",
    "open_fridge+0",
    "open_laptop_lid+0",
    "open_microwave+0",
    "put_money_in_safe+2",
    "open_drawer+1",
    "close_drawer+0",
    "close_grill+0",
]

TASKVARS_TEST_L4 = [
    "push_buttons4+1",
    "push_buttons4+2",
    "push_buttons4+3",
    "take_shoes_out_of_box+0",
    "put_items_in_drawer+0",
    "put_items_in_drawer+2",
    "put_items_in_drawer+4",
    "tower4+1",
    "tower4+3",
    "stack_cups+0",
    "stack_cups+3",
    "put_all_groceries_in_cupboard+0",
]


@dataclass(frozen=True)
class GEMBenchSplit:
    name: str
    taskvars: tuple[str, ...]
    dataset_split: str
    seeds: tuple[int, ...]
    official_level: str


GEMBENCH_SPLITS = {
    "val": GEMBenchSplit(
        name="val",
        taskvars=tuple(TASKVARS_TRAIN),
        dataset_split="val_dataset",
        seeds=(100,),
        official_level="val",
    ),
    "test_l1": GEMBenchSplit(
        name="test_l1",
        taskvars=tuple(TASKVARS_TRAIN),
        dataset_split="test_dataset",
        seeds=(200, 300, 400, 500, 600),
        official_level="L1",
    ),
    "test_l2": GEMBenchSplit(
        name="test_l2",
        taskvars=tuple(TASKVARS_TEST_L2),
        dataset_split="test_dataset",
        seeds=(200, 300, 400, 500, 600),
        official_level="L2",
    ),
    "test_l3": GEMBenchSplit(
        name="test_l3",
        taskvars=tuple(TASKVARS_TEST_L3),
        dataset_split="test_dataset",
        seeds=(200, 300, 400, 500, 600),
        official_level="L3",
    ),
    "test_l4": GEMBenchSplit(
        name="test_l4",
        taskvars=tuple(TASKVARS_TEST_L4),
        dataset_split="test_dataset",
        seeds=(200, 300, 400, 500, 600),
        official_level="L4",
    ),
}


def official_taskvars_for_split(split: str) -> tuple[str, ...]:
    try:
        return GEMBENCH_SPLITS[split].taskvars
    except KeyError as exc:
        raise ValueError(f"Unknown GEMBench split {split!r}; choices={sorted(GEMBENCH_SPLITS)}") from exc
