...
TASK_NAME="gembench_microsteps_9v32_3cam224_1e-4"
...
if [[ ! -x "${PYTHON_BIN}" ]]; then
  echo "[gembench-9v32-train] missing python: ${PYTHON_BIN}" >&2
  exit 1
fi

if [[ ! -f "${GEMBENCH_ROOT}/train_dataset/microsteps.tar.gz" ]]; then
  echo "[gembench-9v32-train] missing train microsteps tar: ${GEMBENCH_ROOT}/train_dataset/microsteps.tar.gz" >&2
  exit 1
fi
if [[ ! -d "${GEMBENCH_ROOT}/train_dataset/keysteps_bbox/seed0" ]]; then
  echo "[gembench-9v32-train] missing train keysteps LMDB: ${GEMBENCH_ROOT}/train_dataset/keysteps_bbox/seed0" >&2
  exit 1
fi
for arg in "$@"; do
  case "${arg}" in
    task=*|+task=*|++task=*|task.*=*|+task.*=*|++task.*=*|data=*|+data=*|++data=*|data.*=*|+data.*=*|++data.*=*)
      echo "[gembench-9v32-train] refusing override that could bypass the 9V/32A dataset contract: ${arg}" >&2
      exit 1
      ;;
  esac
done
...
PYTHONPATH=src "${PYTHON_BIN}" scripts/audit_gembench_microsteps_9v32_cache.py \
  ...
  --rgb-cache-dir "${GEMBENCH_9V32_RGB_CACHE_DIR}" \
  --output-json "${AUDIT_DIR}/microsteps_9v32_cache.json" \
  --output-md "${AUDIT_DIR}/microsteps_9v32_cache.md"
...
WANDB_SUBPROJECT_DEFAULT="${WANDB_SUBPROJECT:-fastwam-gembench-wam-9v32}"
WANDB_RUN_NAME_DEFAULT="${WANDB_RUN_NAME:-${RUN_ID}}"
WANDB_GROUP_DEFAULT="${WANDB_GROUP:-fastwam-gembench-wam-9v32-b4a1}"
