...
export WANDB_GROUP="${WANDB_GROUP:-gembench-wam-9v32-eval10-watchdog}"

python scripts/watch_gembench_policy_eval10_wandb.py \
  --run-dir "${RUN_DIR}" \
  --model-name fastwam \
  --run-id "${RUN_ID}" \
  --eval10-config configs/eval/gembench_official_eval10_periodic_indomain_ood_seed200.yaml \
