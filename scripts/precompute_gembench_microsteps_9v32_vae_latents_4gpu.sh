...
RUN_ID="${RUN_ID:-gembench_9v32_vae_cache_$(date +%Y%m%d_%H%M%S)}"
...
export GEMBENCH_9V32_VAE_CACHE_DIR="${GEMBENCH_9V32_VAE_CACHE_DIR:-${GEMBENCH_ROOT}/fastwam_cache/vae_latents/microsteps_9v32_seed0_3cam224x672_t9_a32_v1}"

IFS=',' read -r -a GPU_IDS <<< "${CUDA_DEVICES}"
if (( ${#GPU_IDS[@]} < NUM_SHARDS )); then
  echo "[9v32-vae-cache] CUDA_VISIBLE_DEVICES has ${#GPU_IDS[@]} ids but NUM_SHARDS=${NUM_SHARDS}: ${CUDA_DEVICES}" >&2
  exit 1
fi

mkdir -p "${LOG_DIR}" "${GEMBENCH_9V32_VAE_CACHE_DIR}"

COMMON_ARGS=(
  --manifest "${GEMBENCH_9V32_MANIFEST}"
  --rgb-cache-dir "${GEMBENCH_9V32_RGB_CACHE_DIR}"
  --cache-dir "${GEMBENCH_9V32_VAE_CACHE_DIR}"
  --num-shards "${NUM_SHARDS}"
)

echo "[9v32-vae-cache] init cache=${GEMBENCH_9V32_VAE_CACHE_DIR} run_id=${RUN_ID}"
...
for shard in $(seq 0 $((NUM_SHARDS - 1))); do
  ...
  echo "[9v32-vae-cache] launching shard=${shard}/${NUM_SHARDS} gpu=${gpu} log=${log}"
...
if (( failed != 0 )); then
  echo "[9v32-vae-cache] one or more shards failed" >&2
  exit 1
fi

echo "[9v32-vae-cache] all shards finished"
"${PYTHON_BIN}" - <<'PY'
import json
import os
...
cache_dir = Path(os.environ["GEMBENCH_9V32_VAE_CACHE_DIR"])
